...
the first sorted singly linked list separated by a single space. 

The second line of the input contains the elements of the second 
...
Node *mergeTwoSortedLinkedLists(Node *head1, Node *head2)
{
   ...
   {
	   // if head1 data is less then connect this node with temp,then shift temp,shift head1 also  	  
	   if(head1 -> data < head2-> data)
	   {
		   temp-> next = head1;
		   temp = head1;
           ...
           // else head3 data is less then connect this node with temp,then shift  temp,shift head2 also
           else 
		   {
             temp->next = head2;
             temp = head2;
