...
*****************************************************************/
void helper(Node *temp,int index,int pos)
{
	if(index < pos -1)
	{
		// when the temp goes out of length of LL
		if(temp == NULL)
		  return ;

		helper(temp-> next,index+1,pos);
	}
	else
	{
	    // iwhen the temp pointer reaches to end of LL
		if(temp -> next != NULL)
