...
then you should return the same linked list without any change. And 
if position 'pos' is equal to length of input linked list, then insert 
...
The first line of each test case or query contains the elements of 
the linked list separated by a single space. 
...
Output format:
For each test case/query, print the resulting linked list of integers 
...
void helper(Node *temp,int i,int count,Node *newNode)
{
	if(count < i -1)
	{
		// This happens when are going to insert a node out of LL length
		if(temp == NULL)
		{
			return;
		}
		helper(temp-> next,i,count+1,newNode);
	} 
	else
	{
